...
Spring创建Bean过程为实例化、属性注入、初始化，在实例化A对象后将A对象工厂放入三级缓存（singletonFactories ）中，再开始注入属性B，开始实例化B，在B发现注入A时，从一到三级缓存查找A，从三级缓存中取得A对象工厂，通过工厂得到未完整的实例A并放入二级缓存，如果是开启了aop的情况下，得到的是A的代理对象，删除三级缓存的A，B就完成实例化并且初始化，将B放入一级缓存，A从一级缓存取得B对象，并将二级缓存的A删除，将初始化好的A放入一级缓存中。

[相关链接](https://blog.csdn.net/weixin_44139651/article/details/124347620) [相关链接](https://www.cnblogs.com/daimzh/p/13256413.html)
